...
typedef struct Ready_Queue_RR_t {
  ...
  Event* queue[MAX_SIZE_ROUND_ROBIN]; //Array of pointers to the event queue array
} Ready_Queue_RR;

//Function Declarations
Ready_Queue_RR* ready_queue_RR_initialize();
int ready_queue_RR_size(Ready_Queue_RR*);
void ready_queue_RR_push(Ready_Queue_RR*,Event*);
Event* ready_queue_RR_pop(Ready_Queue_RR*);
Event* ready_queue_RR_top(Ready_Queue_RR*);
